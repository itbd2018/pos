...
class CategoryController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($request->category_tab == Null) {
            $request->category_tab = 0;
        } else {
            $tabs = Category::where('category_tab', 1)->where('status', 1)->get();
            if (count($tabs) >= 5) {
                Session::flash('error', 'Cannot Add to Tab Section.');
                return back();
            }
        }
        $category->is_featured = $request->is_featured;
        $category->category_tab = $request->category_tab;
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($request->category_tab == Null) {
            ...
            $tabs = Category::where('category_tab', 1)->where('status', 1)->get();
            if (count($tabs) >= 6) {
                Session::flash('danger', 'Cannot Add to Tab Section.');
                // dd('ok');
                return back();
            }
        }
    ...
    public function destroy($id)
    {
        if (!demo_mode()) {
            ...
            $notification = array(
                'message' => 'Brand can not be deleted on demo mode.',
                'alert-type' => 'error'
            );
    ...
    public function checkCategoryTabs()
    {
        return response()->json(Category::where('category_tab', 1)->where('status', 1)->get());
    }
}
